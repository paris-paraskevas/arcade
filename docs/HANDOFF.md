...
2. **Username editor.** New users are `player_<10hex>`; friending is by exact (case-sensitive) username, so people need to set a friendly name (profile RLS update policy exists; no UI). Also fixes the long-username overflow in the mobile header.
